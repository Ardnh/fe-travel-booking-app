import type { CreatePoolDTO, Pagination, Pool, UpdatePoolDTO, PoolParams, Options, AvailablePoolsOptions, PoolInfo, AvailablePoolsByCity } from "~/models";
...
export const usePoolsStore = defineStore("pools", () => {
    ...
    const poolsByVendor = ref<Pool[]>([]);
    const poolsOptionsByVendor = ref<Pool[]>([]);
    const pool = ref<Pool | null>(null);
    const poolLocationsOptions = ref<AvailablePoolsOptions[]>([]);
    const poolLocationsByVendor = ref<AvailablePoolsByCity[]>([]);
    const availablePools = ref<PoolInfo[]>([]);
    const poolsPagination = ref<Pagination>({ ...INITIAL_PAGINATION });
    const poolCityName = ref<string>("");

    const getOriginPoolOptions = computed(() => {
        return poolsOptionsByVendor.value.map((row) => {
            return {
                label: row.name,
                value: row.pool_id,
            };
    ...
    const getDestinationPoolOptions = computed(() => {
        if (poolCityName.value === "") {
            return poolsOptionsByVendor.value.map((row) => {
                return {
                    label: row.name,
                    value: row.pool_id,
        ...
        return poolsOptionsByVendor.value
            .filter((row) => {
                return row.city.includes(poolCityName.value);
            })
            .map((row) => {
                return {
                    label: row.name,
    ...
    const getPoolByVendorID = async (params: PoolParams) => {
        ...
        if (result.success) {
            poolsByVendor.value = result.data;
    ...
    const getPoolByVendorIdOptions = async () => {
        ...
        const vendorId = vendor.value.vendor_id;
        const result = await run("getPoolByVendorID", () => service.getPoolByVendorID(vendorId, { page: 1, page_size: 50 }));

        if (result.success) {
            poolsOptionsByVendor.value = result.data;
        }
    };
    ...
    const setPoolCityName = (name: string) => {
        poolCityName.value = name;
    };
    ...
    return {
        ...
        getOriginPoolOptions,
        getDestinationPoolOptions,
        getAvailableLocationsByVendorID,
        poolLocationsOptions,
        poolsOptionsByVendor,
        poolLocationsByVendor,
        availablePools,
        setPoolCityName,
    };
});
