...
const { poolOptions, poolLocations } = storeToRefs(poolsStore);
const { getPoolByVendorIdOptions, getAvailableLocationsByVendorID } = poolsStore;
...
const destinationMode = ref<"city" | "pools">(props.data?.destinationMode ?? "city");
const destinationCity = ref(props.data?.destinationCity ?? "");
const destinationPoolIds = ref<string[]>(props.data?.destinationPoolIds ?? []);
const departureTimes = ref<string[]>(props.data?.departureTimes ?? ["06:00", "09:00", "14:00", "20:00", "22:00"]);
const newTime = ref("");
const validFrom = ref(props.data?.validFrom ?? "2026-07-01");
const validTo = ref(props.data?.validTo ?? "2026-09-14");
const daysOfWeek = ref<number[]>(props.data?.daysOfWeek ?? [0, 1, 2, 3, 4, 5, 6]);
...
const resolvePrice = (t: string) => bands.value.find((band) => inBand(t, band.from, band.to))?.price ?? 0;
...
const toggleDay = (d: number) => {
    daysOfWeek.value = daysOfWeek.value.includes(d) ? daysOfWeek.value.filter((x) => x !== d) : [...daysOfWeek.value, d].sort();
};
const addBand = () => bands.value.push({ label: "", from: "", to: "", price: 0 });
const removeBand = (i: number) => bands.value.splice(i, 1);

// ── Turunan / preview ──────────────────────────────────────────────────
const totalSeat = computed(() => layouts.find((l) => l.value === layoutId.value)?.seats ?? 0);

const destinationPools = computed(() => {
    if (destinationMode.value === "city") return poolsByCity[destinationCity.value] ?? [];
    return (poolsByCity[destinationCity.value] ?? []).filter((p) => destinationPoolIds.value.includes(p.value));
});
...
const scheduleCount = computed(() => departureTimes.value.length * destinationPools.value.length * matchingDates.value.length);

const dayLabel = computed(() => (daysOfWeek.value.length === 7 ? "Setiap hari" : `${daysOfWeek.value.length} hari/minggu`));
...
const onOpenLocationMenu = async () => {
    if (poolLocations.value.length == 0) {
...
<template>
    ...
    >
            ...
            <div class="mx-auto flex max-w-3xl flex-col gap-4">
                ...
                <UCard variant="outline">
                        ...
                        <UFormField label="Tujuan">
                            <UFieldGroup class="w-full">
                                <UButton
                                    ...
                                    :color="destinationMode === 'city' ? 'primary' : 'neutral'"
                                    :variant="destinationMode === 'city' ? 'solid' : 'outline'"
                                    @click="destinationMode = 'city'"
                                />
                                <UButton
                                    label="Pilih pool"
                                    class="flex-1 justify-center"
                                    :color="destinationMode === 'pools' ? 'primary' : 'neutral'"
                                    :variant="destinationMode === 'pools' ? 'solid' : 'outline'"
                    ...
                    <UFormField label="Kota tujuan" class="mt-4">
                        <USelectMenu v-model="destinationCity" :items="poolLocations" class="w-full" @update:open="onOpenLocationMenu" placeholder="Select city" />
                    </UFormField>
                    ...
                    <p v-if="destinationMode === 'city'" class="mt-3 flex items-center gap-1.5 text-sm text-primary">
                        <UIcon name="i-lucide-info" />
                        Akan dibuat jadwal ke
                        {{ (poolsByCity[destinationCity] ?? []).length }} pool aktif di {{ destinationCity }}.
                    </p>
                </UCard>
                ...
                <UCard variant="outline">
                    ...
                    <UFormField label="Jam keberangkatan">
                        <div class="flex flex-wrap items-center gap-2">
                            <UBadge v-for="t in departureTimes" :key="t" color="neutral" variant="subtle" class="gap-1">
                                {{ t }}
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        ...
                        <UFormField label="Sampai">
                            <UInput v-model="validTo" type="date" class="w-full" />
                        </UFormField>
                    </div>
                    ...
                    <UFormField label="Pengulangan" class="mt-4">
                        <div class="flex flex-wrap items-center gap-1.5">
                            <UButton
                                ...
                                :color="daysOfWeek.includes(i) ? 'primary' : 'neutral'"
                ...
                <UCard variant="outline">
                    ...
                    <div class="grid items-end gap-4 sm:grid-cols-[2fr_1fr]">
                        <UFormField label="Layout kursi">
                            <USelect v-model="layoutId" :items="layouts" class="w-full" />
                        </UFormField>
                ...
                <UCard variant="outline">
                    ...
                    <div v-for="(band, i) in bands" :key="i" class="mb-2 grid grid-cols-[1.3fr_0.8fr_0.8fr_1.2fr_auto] items-center gap-2">
                        ...
                        <UButton icon="i-lucide-trash" color="neutral" variant="ghost" :disabled="bands.length <= 1" @click="removeBand(i)" />
                    </div>

                    <p v-if="bands.some((b) => b.from && b.to && timeToMin(b.from) > timeToMin(b.to))" class="mt-1 flex items-center gap-1.5 text-xs text-warning">
                ...
                <div class="rounded-lg bg-primary-50 p-4 dark:bg-primary-950/40">
                    ...
                    <p class="mb-3.5 text-sm text-primary/80">{{ departureTimes.length }} jam × {{ destinationPools.length }} pool tujuan × {{ matchingDates.length }} hari</p>
        ...
        <template #footer>
            <UButton label="Batal" color="neutral" variant="ghost" @click="close" />
            <UButton label="Generate jadwal" icon="i-lucide-zap" :disabled="!scheduleCount" @click="submit" />
        </template>
    </UModal>
</template>
